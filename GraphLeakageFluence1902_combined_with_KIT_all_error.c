...
const Int_t i = 1;	
	...
	TMultiGraph *combined = new TMultiGraph();
		...
		SetA->GetXaxis()->SetTitle("Fluence (p/cm^2)");
		...
		SetB->GetXaxis()->SetTitle("Fluence (p/cm^2)");
		...
		mg->Add(kit,"p");
		mg->Draw("psame");
		mg->GetXaxis()->SetTitle("Fluence (p/cm^2)");
		mg->GetYaxis()->SetTitle("Change in Leakage Current (-nA)");	
		mg->SetTitle("");

	   TLegend *legend = new TLegend(0.15,0.75,0.25,0.85);
		legend->AddEntry(SetA,"Set A","p");
		legend->AddEntry(SetB,"Set B","p");
		legend->AddEntry(kit,"KIT","p");
   		legend->Draw();	
	
}

void GraphLeakageFluence1902_combined_with_KIT_all_error()
{	
