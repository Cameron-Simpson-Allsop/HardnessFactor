...
double ExtractNonIrradiatedCurrent_Inon(TString txtName="", TString graphTitle ="", double voltage = 91.)
{
	...
	TF1* fit1 = new TF1("fit1","pol1", 50,300); //Fits 1st order poly within range
...
CurrentWarmSetA ExtractIrradiatedCurrentWA(TString txtName="", TString graphTitle="", double voltage = 91.)//Function to fit curves and extract current from fits at user given voltage.
{
	...
	TF1* fit1 = new TF1("fit1","pol1", 75,175); //Fits 1st order poly within range
...
void Isidre_Hardness_Factor()
{	
	rootlogonATLAS();
	EvaluateDiodes();
	std::cout << "Note: For the neutron equivalent current related damage rate, the value from Moll's thesis was used. What did Isidre use?" << std::endl;
}
