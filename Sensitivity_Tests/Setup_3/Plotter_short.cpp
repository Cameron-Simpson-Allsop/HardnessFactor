...
void Plotter_short()
{
  ...
  for(int i{0}; i<files.size(); ++i)
      ...
      while(!inFile.eof())
	{
	  ...
	  getline(inFile,line);
	    if(linecount>36 && line != "END" && linecount%2 != 0 && linecount<190)
	      {
  ...
  mg->Draw("AP");
  leg->Draw();
  canvas->SaveAs("BPW34F_setup3_short.root");
  canvas->SaveAs("BPW34F_setup3_short.pdf");
  canvas->SaveAs("BPW34F_setup3_short.png");
}
